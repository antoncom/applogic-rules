...
function loadvar_subscribed:load(varname, rule)
	...
	local match = varlink.source.match or {}
	...
	if noerror then
		...
		-- А также функцию проверки is_lived_twice() показывающую что данное значение "живет" в переменной уже 2 итерации
				...
				print("================")
				print("| loadvar_subscribed.queue length: " .. tostring(#loadvar_subscribed.queue))
				print("| Rule iteration: " .. tostring(rule.iteration))
				print("================")
