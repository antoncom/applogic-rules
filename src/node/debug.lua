...
function debug:operator_bash(op_name, body, result, noerror)
    ...
    if result.stderr then
        dnlink.operator[op_name].value = result.stderr .. "\n" .. dnlink.operator[op_name].value
    end
    debug:set_noerrors(dnlink, noerror)
end
...
local metatable = {
	__call = function(table, nodename, rule)
        table.nodename = nodename
